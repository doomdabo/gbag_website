...
def visualize_result(pci,azi,down,predictions):
    building_path = 'C:\GBAG_WEB\\web_tuto_GBAG\\gbag\static\\building\\Cropped.shp'
    jangbi_path = 'C:\GBAG_WEB\\web_tuto_GBAG\\gbag\\static\\csv\\maps\\jangbi.csv'
    building = gpd.read_file(building_path)
    jangbi = pd.read_csv(jangbi_path, encoding='cp949')
    plt.rcParams["figure.figsize"] = (7, 5) # 그림 크기 설정

    numeric_data_test = pd.read_csv('C:\GBAG_WEB\\web_tuto_GBAG\\gbag\\static\\csv\\test\\grid_5.csv')    # 파일 읽기
    ...
    ax.text(325885 , 4151285, box, bbox=boxstyle, fontsize=15)
    ...
    ### 이미지 저장 코드
    fig.savefig('C:\GBAG_WEB\\web_tuto_GBAG\\gbag\\static\\img\\result_image.png', dpi=250)